...
namespace KeySplineAnimations
{
    public partial class SplineExample : Page
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            collection.AddYazýOzellik(new YazýOzellik("Century Gothic", 14));
            collection.AddYazýOzellik(new YazýOzellik("Frnklin Gothic", 16));
            collection.AddYazýOzellik(new YazýOzellik("Elephant", 18));
            collection.AddYazýOzellik(new YazýOzellik("HarlowSolid", 20));
            collection.AddYazýOzellik(new YazýOzellik("Century", 22));
            collection.AddYazýOzellik(new YazýOzellik("Anurag", 24));

            AbsIterator ýterator = collection.CreateIterator();

            MessageBox.Show("Word Yazý Tipleri:");

            for (YazýOzellik yazý = ýterator.First(); !ýterator.isComplete; yazý = ýterator.Next())
            {
                MessageBox.Show("Yazý Boyutu :"+ yazý.Boyut +" Yazý Tipi :" + yazý.YazýTip);
            }
